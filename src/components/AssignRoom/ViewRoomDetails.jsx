...
import { db, RoomRef, userRef, rtdb } from "@/lib/firebase"; // Import RTDB instance
import { ExclamationTriangleIcon } from "@heroicons/react/24/outline";
import { doc, getDocs, query, updateDoc, where, increment } from "firebase/firestore";
import { ref, remove } from "firebase/database"; // Import RTDB remove function
...
export function EditRoom({ data }) {
  ...
  const usassign = async () => {
    ...
    try {
      ...
      // Remove the assignment details from the RTDB
      const roomAssignmentRef = ref(rtdb, `roomAssignments/${data.RoomId}`);
      await remove(roomAssignmentRef);
